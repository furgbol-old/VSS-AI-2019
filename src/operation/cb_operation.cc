...
CBOperation::CBOperation(bool *running, bool *changed, vss::Ball *ball, world_model::Robot *robot, int side, int max_velocity) :
...
void CBOperation::configure() {
    ...
    robot_->id = json_file["robots"]["centerback"]["id"];
    max_ball_distance_ = json_file["robots"]["centerback"]["max ball distance"];

    switch (side_) {
        case LEFT:
            field_line_.setX(json_file["robots"]["centerback"]["field line"]["x"]);
            break;
        case RIGHT:
            field_line_.setX(170 - (float)json_file["robots"]["centerback"]["field line"]["x"]);;
            break;
    }
    field_line_.setMinY(json_file["robots"]["centerback"]["field line"]["min y"]);
    field_line_.setMaxY(json_file["robots"]["centerback"]["field line"]["max y"]);
}

void CBOperation::printConfigurations() {
    std::cout << "[STATUS]: Centerback operator configuration done!" << std::endl;
...
void CBOperation::verifyPosition() {
    float robot_x;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        robot_x = robot_->x;
...
bool CBOperation::canKick(float robot_x, float robot_y, float ball_x, float ball_y) {
    switch (side_) {
        case LEFT:
...
bool CBOperation::outOfAngle(float robot_angle) {
    if (target_angle_ == 0) {
...
bool CBOperation::outOfTarget(float robot_x, float robot_y, float ball_x, float ball_y) {
...
void CBOperation::setKick(float robot_y, float ball_y) {
    linear_velocity_ = 0;
...
void CBOperation::fixesAngle(float robot_angle) {
    linear_velocity_ = 0;
...
void CBOperation::goToTarget(float robot_x, float robot_y, float ball_x, float ball_y) {
    angular_velocity_ = 0;
    if (out_of_place_ == NO) {
