...
CBOperation::CBOperation() : sending_queue_() {}

CBOperation::CBOperation(world_model::Robot *robot, vss::Ball *ball, bool *running, bool *status_changed) : robot_(robot), ball_(ball),
...
std::queue<std::vector<uint8_t>> CBOperation::getSendingQueue() { return sending_queue_; }

void CBOperation::setConfigurations() {
    std::cout << "[STATUS]: Configuring centerback operator..." << std::endl;
    ...
    field_line_.setMinY(json_file["robots"]["centerback"]["field_line"]["min_y"]);
    field_line_.setMaxY(json_file["robots"]["centerback"]["field_line"]["max_y"]);
    max_ball_distance_ = json_file["robots"]["centerback"]["max_ball_distance"];
    max_velocity_ = json_file["robots"]["centerback"]["max_velocity"];
}
